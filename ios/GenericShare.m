...
- (void)shareSingle:(NSDictionary *)options
    ...
    if([SLComposeViewController isAvailableForServiceType:serviceType]) {
        ...
        NSString *escapedMessage = [options[@"message"] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLHostAllowedCharacterSet]];
        ...
        if ([options[@"social"] isEqualToString:@"twitter"]) {
            NSString *URL = [NSString stringWithFormat:@"https://twitter.com/intent/tweet?text=%@&url=%@", escapedMessage, escapedUrl];
            [self openScheme:URL];
        }
        
        if ([options[@"social"] isEqualToString:@"facebook"]) {
            NSString *URL = [NSString stringWithFormat:@"https://www.facebook.com/sharer/sharer.php?u=%@", escapedUrl];
            [self openScheme:URL];
        }
